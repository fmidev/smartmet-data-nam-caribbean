...
UTCHOUR=`date -u +%H -d '-1 hours'`
RUN=`expr $UTCHOUR / 6 \* 6`
RUN=`printf %02d $RUN`
DATE=`date -u +%Y%m%d${RUN}00 -d '-1 hours'`
RUNDATE=`date -u +%Y%m%d -d '-1 hours'`
TMP=/smartmet/tmp/data/nam/$AREA/$DATE
LOGFILE=/smartmet/logs/data/nam${RUN}.log
...
function downloadStep()
{
    ...
    step=$(printf '%02d' $1)
    FILE="nam.t${RUN}z.afwaca${step}.grb2.tm00"
